`timescale 1ns/1ps
module tb_mapper682_irq_sequence;
 reg clk=0;
 always #5 clk=~clk;
...
initial begin
 ...
 // Execute the original CPU Cycle IRQ test through its count result.
 // Redirect only the following display/UI code to the scanline test setup.
 ...
 if(ram['h67]!=1 || ram['h66]!=5)$fatal(1,"CPU Cycle IRQ result=%0d, IRQ count=%0d",ram['h66],ram['h67]);
 if(dut.cpu_irq_en || dut.cpu_irq_pending)$fatal(1,"CPU IRQ remains active");
 if(ram[2042]<6)$fatal(1,"Scanline test stopped after CPU test, PC%h",regs[63:48]);
 i=ram[2043];repeat(1200)@(negedge clk);
 if(ram[2043]==i)$fatal(1,"Mainline stopped");
 $display("PASS original CPU Cycle IRQ result=5; IRQ disabled; subsequent scanline handlers and mainline run");
 $finish;
end endmodule
