`timescale 1ns/1ps
module tb_mapper682_vector_irq;
 reg clk=0;
 always #5 clk=~clk;
...
initial begin
 ...
 ram['h304]=8'h4c;ram['h305]=8'h65;ram['h306]=8'hcf;
 enable=1;reset_n=1;
 cycles=0;
 while(regs[63:48]!=16'hcf96 && cycles<100000)begin @(negedge clk);cycles=cycles+1;end
 if(regs[63:48]!=16'hcf96 || ram['hb9]!=8'h55)$fatal(1,"Vector IRQ stalled PC=%h result=%h en=%b pending=%b",regs[63:48],ram['hb9],dut.cpu_irq_en,dut.cpu_irq_pending);
 $display("PASS original Vector Redirect IRQ code and handler");$finish;
end
endmodule
